/// `mastro migrate` — generate and run a migration script.
///
...
import mastro/cli/types.{NoDb, Postgres, Sqlite}
import simplifile
    ...
    _ -> {
      ...
      case simplifile.read_directory(migrations_dir) {
        Error(_) -> {
          ...
          io.println("Generate a resource first: mastro gen resource ...")
        }
        Ok(files) -> {
          let sql_files =
            files
